function initializeDashboard(){
  ...
  for(const section of document.querySelectorAll('main > .tab')){
    ...
    function clearClickPick(){
      ...
      activeClickPick.el.classList.remove('cardPicked');
      activeClickPick.el.querySelector('.cardDragGrip')?.removeAttribute('aria-pressed');
      activeClickPick=null;suppressClick=false;toolbarHint.textContent='Drag a title bar: left or right for half a row; center for a full row.';
    }
    const records=candidates.map(({el,width},index)=>{
      ...
      const grip=document.createElement('button');grip.type='button';grip.className='cardDragGrip';grip.textContent='⠿ '+title;grip.title='Drag to move this section';grip.setAttribute('aria-label','Drag section: '+title);grip.draggable=false;
      ...
      grip.addEventListener('click',e=>{
        if(activePointerDrag)finishPointerDrag({pointerId:activePointerDrag.pointerId});
        if(suppressClick){suppressClick=false;return;}
        e.preventDefault();e.stopPropagation();
        if(activeClickPick){
          if(activeClickPick.el!==el)grid.insertBefore(activeClickPick.el,el);
          clearClickPick();save();return;
        }
        activeClickPick={el,grid};el.classList.add('cardPicked');grip.setAttribute('aria-pressed','true');toolbarHint.textContent='Click another section handle to drop this tile';
      });
      grip.addEventListener('mousedown',e=>{
        if(e.button!==0||!activeClickPick)return;
        if(activeClickPick.el===el){clearClickPick();suppressClick=true;return;}
        grid.insertBefore(activeClickPick.el,el);clearClickPick();save();suppressClick=true;e.preventDefault();e.stopPropagation();
      });
      bar.addEventListener('pointerdown',e=>{
        if(e.target.closest('.cardLayoutActions'))return;
        if(e.button!==0||activePointerDrag)return;
        if(activeClickPick)return;
        clearClickPick();
        e.stopPropagation();
        ...
        activePointerDrag={el,grid,grip,placeholder,pointerId:e.pointerId,offsetX:e.clientX-rect.left,offsetY:e.clientY-rect.top,startX:e.clientX,startY:e.clientY,moved:false,pickOnClick:!!e.target.closest('.cardDragGrip')};
      ...
      grip.addEventListener('pointerup',e=>{
        if(suppressClick||!activeClickPick)return;
        if(activeClickPick.el===el){clearClickPick();suppressClick=true;return;}
        grid.insertBefore(activeClickPick.el,el);clearClickPick();save();suppressClick=true;e.preventDefault();e.stopPropagation();
      });
    ...
    document.addEventListener('keydown',e=>{if(e.key==='Escape')clearClickPick()});
  }
}
