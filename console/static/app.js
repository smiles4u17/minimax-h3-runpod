...
async function applySamimateSourceDefaults(sourcePath){
  ...
  try{
    ...
    let fields=[['samimate_width',info.width,832],['samimate_height',info.height,480],['samimate_fps',info.fps,16]];
    let changed=[];
    for(const [id,next,generic] of fields){
      ...
      if(!cur||cur===String(generic)||cur===last){
...
function updateSamimateBackendUI(){
  ...
  $('samimate_backend_hint').textContent=h3?'Ref2VA replaces the SAM area using all identity views. The original background and audio are composited back. Select a 1–15 second trim; H3 uses 24 fps. Uses 20-step Ref2VA with native masked sampling. Model and identity LoRAs come from the H3 tab.':'WanAnimate receives the SAM subject mask as its edit area, then the original background is composited back.';
...
async function submitRun(url,p,target='inf'){
  try{
    ...
    if(r.debug)log('Debug: '+JSON.stringify(r.debug));
...
async function restoreTabPreviews(tab,force=false){
  ...
  try{
    ...
    if(tab==='samimate')updateTimeline('samimate');
...
function currentPayload(tab){
  ...
  return samimateGenerationBackend()==='h3'?samimateH3Payload():samimateWanPayload(val('sam_mask_video_path')||'');
}
...
async function refreshRecentOutputs(announce=true){try{let r=await api('/api/outputs/recent?limit=12');let el=$('recent_outputs');if(el)showOutputGrid(el,r.items||[]);if(announce)log('Recent outputs refreshed')}catch(e){if(announce)log('Recent outputs error: '+e.message)}}
...
function selectH3TurboFamily(family){set('h3_turbo_family',family);set('h3_sampling_preset','custom');if(family==='larry'){fillH3ModelSelect('h3_turbo_lora',[...Array.from($('h3_turbo_lora').options,o=>o.value),'minimax_h3_turbo_v4_step600_ema.safetensors']);set('h3_turbo_lora','minimax_h3_turbo_v4_step600_ema.safetensors');set('h3_sampler','h3_turbo');set('h3_scheduler','simple')}else if(family==='lightx2v'){let ref=val('h3_task')==='r2v',current=val('h3_turbo_lora'),name=ref?'minimax_h3_ref2v_turbo_4step_v0.1_comfyui_bf16.safetensors':current.includes('_fl2v_')&&current.includes('_comfyui_')&&!current.includes('_sla_')?current:'minimax_h3_fl2v_turbo_4step_v1.0_768p_comfyui_bf16.safetensors';fillH3ModelSelect('h3_turbo_lora',[...Array.from($('h3_turbo_lora').options,o=>o.value),name]);set('h3_turbo_lora',name);set('h3_sampler','euler');set('h3_scheduler','simple')}updateH3TurboUI(false);persistSettingsSoon()}
