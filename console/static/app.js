...
function currentH3TurboLora(name){return String(name||'').replace(/minimax_h3_fl2v_turbo_4step_(?:v0\.1_768p_sla|v1\.[01]_768p)_comfyui_bf16\.safetensors$/, 'minimax_h3_fl2v_turbo_4step_v1.2_768p_comfyui_bf16.safetensors').replace(/minimax_h3_fl2v_turbo_8step_v1\.0_comfyui_bf16\.safetensors$/, 'minimax_h3_fl2v_turbo_8step_v1.0_768p_comfyui_bf16.safetensors')}
function fillH3ModelSelect(id,names=[]){let select=$(id);if(!select)return;let current=id.endsWith('turbo_lora')?currentH3TurboLora(select.value):select.value;if(id.endsWith('turbo_lora'))names=(names||[]).map(currentH3TurboLora);let unique=[...new Set((names||[]).filter(Boolean))].sort((a,b)=>a.localeCompare(b));select.replaceChildren();if(current&&!unique.includes(current))unique.unshift(current);for(const name of unique){let option=document.createElement('option');option.value=name;option.textContent=name+(name===current&&!names.includes(name)?' (saved; not listed)':'');select.appendChild(option)}if(current)select.value=current}
...
function watch(endpoint,job,target='inf',meta={}){
  if(target==='h3'&&window.trackCurrentH3Job)window.trackCurrentH3Job(endpoint,job);
  ...
  async function poll(){
    ...
    try{
      ...
      if(['COMPLETED','FAILED','CANCELLED','TIMED_OUT'].includes(s)){
        if(target==='h3'&&window.finishCurrentH3Job)window.finishCurrentH3Job(endpoint,job);
...
function selectH3TurboFamily(family){set('h3_turbo_family',family);set('h3_sampling_preset','custom');if(family==='larry'){fillH3ModelSelect('h3_turbo_lora',[...Array.from($('h3_turbo_lora').options,o=>o.value),'minimax_h3_turbo_v4_step600_ema.safetensors']);set('h3_turbo_lora','minimax_h3_turbo_v4_step600_ema.safetensors');set('h3_sampler','h3_turbo');set('h3_scheduler','simple')}else if(family==='lightx2v'){let ref=val('h3_task')==='r2v',current=currentH3TurboLora(val('h3_turbo_lora')),name=current.toLowerCase().includes(ref?'_ref2v_':'_fl2v_')&&current.toLowerCase().includes('_comfyui_')&&!current.toLowerCase().includes('_sla_')?current:ref?'minimax_h3_ref2v_turbo_4step_v0.1_comfyui_bf16.safetensors':'minimax_h3_fl2v_turbo_4step_v1.2_768p_comfyui_bf16.safetensors';fillH3ModelSelect('h3_turbo_lora',[...Array.from($('h3_turbo_lora').options,o=>o.value),name]);set('h3_turbo_lora',name);set('h3_sampler','euler');set('h3_scheduler','simple')}updateH3TurboUI(false);persistSettingsSoon()}
...
function setupSamimateH3Controls(){
  ...
  $('samimate_h3_turbo_enabled').addEventListener('change',()=>{if(!chk('samimate_h3_turbo_enabled')&&val('samimate_h3_sampler')==='h3_turbo')set('samimate_h3_sampler','res_multistep')});
