...
(() => {
  ...
  function sync(change=false){
    ...
    if(change&&active){set('h3_pass1_split',fflf?3:6);set('h3_steps',fflf?4:8);set('h3_megapixels',.2);setChk('h3_turbo_enabled',true);selectH3TurboFamily(chk('h3_use_larry')?'larry':'lightx2v')}
  ...
  $('h3_latent_upscale').onchange=()=>{sync();persistSettingsSoon()};
  const original=h3Settings;
  window.h3Settings=()=>{const legacy=original();const settings={...legacy,legacy_inputs:{first_frame_path:legacy.first_frame_path,last_frame_path:legacy.last_frame_path,reference_paths:legacy.reference_paths,reference_subjects:legacy.reference_subjects},workflow_variant:val('h3_workflow_variant'),use_multi_image:chk('h3_use_multi_image'),use_larry:chk('h3_use_larry'),latent_upscale:chk('h3_latent_upscale'),rtx_upscale:chk('h3_rtx_upscale'),final_megapixels:Number(val('h3_final_megapixels')),pass1_split:Number(val('h3_pass1_split')),second_pass_sigma:Number(val('h3_second_pass_sigma')),latent_upscale_model:val('h3_latent_upscale_model'),photo_paths:[1,2,3,4].map(i=>val('h3_photo'+i).trim()),keyframe_positions:[1,2,3,4].map(i=>val('h3_keyframe'+i).trim())};
  ...
  window.applyH3Settings=(h3={},storage={})=>{
    ...
    for(const [name,defaultValue] of Object.entries({final_megapixels:.9,pass1_split:3,second_pass_sigma:2,latent_upscale_model:'minimax_h3_latent_upscaler_3d_bf16.safetensors'}))set('h3_'+name,h3[name]??defaultValue);
