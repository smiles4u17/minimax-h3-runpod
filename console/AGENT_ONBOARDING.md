...
## GitHub push authorization: 2026-09-22
The user explicitly confirmed https://github.com/smiles4u17/minimax-h3-runpod.git is trusted and instructed "always push for me". This supersedes the preceding pending-approval note. Push completed project changes to this existing repository without asking again; this authorization is scoped to this project and destination. The cancel-button/tile-gesture commits are ready for push; verify the push result before reporting publication.

## Live slow-generation diagnosis: 2026-09-22 local / September 23 UTC
Read-only investigation matched user logs to job 918ea02b-a89d-415f-bead-618cbd780cd4-u1, worker 1z5zsx1d555asj, image tag 7dff6febb. RunPod reports COMPLETED, executionTime 1,185,321 ms and delayTime 59,196 ms; outputs include MP4 and final-frame PNG. This verifies export completion, not visual quality. Direct container streaming timed out twice; durable /api/job/{endpoint}/{job}/diagnostics returned the matching complete logs and timestamps.
Returned metadata confirms attention=sage, RTX 5090, 31.84 GiB VRAM, minimum_vram profile, beta5 diffusion model, LCM/simple, six low-res steps then five high-res steps (second_pass_sigma=3), latent target 1 MP and RTX enabled. Logs explicitly show the H3 Sage patch; the separate MinimaxH3-3D upscaler's "Attn: forced off" is not the H3 attention setting. Both diffusion passes share the attention-patched model chain in workflow_variants.py.
Observed steady sampling times: approximately 29.4 sec/step low-res and 113.7 sec/step high-res, plus roughly two minutes initialization per pass. High-res sampling node timestamps span about 9m48s including initialization/first step; the progress-bar timer resets, so do not add its initialization and final totals blindly. Full prompt executed in 19m42s. Staged H3 weights are 38,444 MB, exceeding available GPU VRAM before activations; memory pressure/offloading is a plausible contributor, not a measured PCIe bottleneck. No evidence supports disabling Sage or forcing a maximum-speed memory profile on this 32 GB worker. Model paths and volume-only diagnostics confirm existing /runpod-volume/models usage and no duplicate volume/model download for this job.
Live endpoint still uses only ADA_32_PRO, existing volume vgc3ky6r6y, max workers 1; current worker snapshot THROTTLED after job completion. No GPU pool, memory profile, model, sampler or active job was changed. Next optimization should measure stage times/VRAM/offload before tuning; lower second-pass sigma count or disable latent upscale only as an explicit quality/speed tradeoff. Update onboarding after any follow-up and preserve this evidence versus older unverified export notes.
