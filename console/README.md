...
The console checks both configured output volumes in the background on server launch, page reload, and **Recent Outputs → Refresh**. Its first check starts at midnight today in the server's local timezone. Later checks resume from the previous successful check, retry failures, and restore missing files previously tracked by the check. Existing local media is recognized by content; recovered files appear under `Output Dir/recovered` and in Recent Outputs. Progress appears above the gallery. The durable checkpoint lives in `~/.runpod_media_console_web/output_sync.json`; partial downloads never appear as finished media. Only media under output prefixes is scanned, excluding internal `_setup` directories. Outputs that were never persisted to a configured volume cannot be recovered by this check.


SAMimate with the **MiniMax H3** backend now has independent model, sampler, step, Turbo, attention, delivery, and identity-LoRA settings. It always submits masked Ref2V. Use **Prepare / reuse masks** to inspect the subject mask and its inverse, then **Send to H3**. Source content, trim/crop, frame caps, and segmentation settings identify reusable masks; changing a generation prompt or sampler does not re-segment. **Regenerate masks** explicitly creates a new pair. Durable manifests and prepared sources live in `cache/samimate_masks`; the local mask gallery also displays earlier SAM history entries (older entries are view-only because their preparation settings are unknown).

**Build prompt and edit** fills the prompt editor and selects exact custom prompt mode. Auto resolution fits the source or crop to the 32-pixel H3 grid and 2-megapixel limit; compositing retains the source canvas. Recent uploads can be selected across pages. **Payload Inspector → Preview payload** builds the full H3 worker input without uploading or submitting; pending local media transport is explicitly marked. After submission, the inspector receives the actual worker input, with large base64 media represented by its character count. LoRA/task mismatches are rejected before media upload.
