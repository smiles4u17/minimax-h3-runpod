...
`use_larry: true` forces the Larry loader and h3_turbo sampler; false uses the task-matched selectable `turbo_lora` and regular `sampler`. Additional `loras` form one common model chain. Model, CLIP, VAEs, prompt, duration, megapixels, aspect_ratio, seed, attention and cache use the established preset fields.
...
## Simplified output selector
The console now uses output_mode: `none` (1 — Not scaled), `latent` (2 — Latent upscaled), or `rtx` (3 — RTX upscale). These choices are exclusive: none omits both upscalers; latent runs the second pass only; rtx runs the latent upscale and second pass, then applies RTX to the decoded result (1920x1080 at 16:9). Explicit output_mode takes precedence over the older boolean flags. Requests without output_mode retain boolean compatibility. The UI uses concise MP / Upscale MP labels, defaults generation to 0.2 MP and latent target to 1 MP, and omits the verbose pixel-count summary. Both resolutions are editable; the latent target is active in both Latent upscaled and RTX modes. Saved explicit values are retained until a workflow preset is selected.
